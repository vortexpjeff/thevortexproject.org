...
Before approval, record a structured assessment with `scripts/record-publication-assessment.sh`. Its coordinator acquires the shared repository `flock`; the worker verifies that the configured live `flock` process is its direct parent, so an environment flag cannot bypass coordination. The assessment preserves who or what performed the checks, a UTC timestamp, source URLs, and separate claim, privacy, and rights bases while the record remains in `review`. Approval is the one explicit human publication decision: the promoter requires the passed assessment, records the internal accountable editor and approval timestamp, changes privacy and rights states to `public-cleared`, and binds the exact reviewed revision. It does not release the article. Released promotion separately requires a real UTC RFC3339 `--published-at` value.

The public projection recursively removes `accountable_editor`, `publication_approval`, `review_assessment`, legacy clearance objects, and nested `editor` or `reviewer` fields. It derives the public lifecycle statement `Claim, privacy, and rights checks passed; human-approved for publication.` rather than exposing stale internal review-era wording. Public pages state that the revision was human-approved without naming the person. The internal catalog and Git history retain accountability.
